...
def test_update_user_data_endpoint():
    #ARRANGE
    client = TestClient(app)
    JWT_SECRET = config("JWT_SECRET")
    jwt_aud="kbe-aw2022-frontend.netlify.app"
    ...
    #ACT
    response = client.patch(f"/users/{new_user_id}", json=updated_test_user)
    #ASSERT
    assert response.status_code == 204
    assert client.get(f"/users/{new_user_id}").json() == updated_test_user
    #CLEANUP
    client.delete("/users", json={"password":"testtesttest4"}, headers={"userId":new_user_id})
...
def test_update_user_data_endpoint_fails_user_not_found():
    ...
    #ACT
    response = client.patch("/users/invalid_id", json=updated_test_user)
    #ASSERT
    assert response.status_code == 404
    assert response.json() == expected_error
